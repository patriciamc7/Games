...
void main()
{
    ...
    v *= g;
    vec4 Ti = texture2D(u_texture2,.02*v.xy+uv * u_texture_tiling)*1.-.2;//water
    vec4 Tf = texture2D(u_texture,.02*v.xy+uv * u_texture_tiling); 
    vec3 T=Ti.rgb;
    //T = Ti.rgb+(1.-Ti.rgb)*(Tf.rgb*0.5); 
    
    vec3 T1,T2;
    T1 = vec3(0.,0.,1.); T1 *= .2*(T+1.);
    T2 = vec3(0.8,0.8,0.8); //T2 = 1.2*Ti*vec3(0.,.0,.0)-.2;
    v = mix(mix(T1+(T2*0.2),0.5*Tf.rgb,.5),T2,T*0.5);   
